...
const pickBestVideoSource = (videoEl) => {
	let variants;
	try {
...
var Tween =  function(_Animation2) {
  ...
  function Tween2(targets, vars, position, skipInherit) {
    ...
    _this3._targets = parsedTargets.length ? _harness(parsedTargets) : _warn("GSAP target " + targets + " not found. https:
    _this3._ptLookup = [];
...
var _win$3, _doc$3, _docElement, _pluginInitted, _tempDiv, _recentSetterPlugin, _reverting, _windowExists$2 = function _windowExists22() {
  ...
  var e = _doc$3.createElementNS ? _doc$3.createElementNS((ns || "http:
  ...
  var owner = target.ownerSVGElement, svg = _createElement("svg", owner && owner.getAttribute("xmlns") || "http:
...
function menuInit() {
  ...
  if (!btn || !menu) return;
  const getNodes = () => {
    const infoContainer = menu.querySelector(INFO_CONTAINER_SELECTOR) || 
...
function ensureUnderlinePathUnderInfo(card) {
  ...
  if (!svg) {
    svg = document.createElementNS("http:
