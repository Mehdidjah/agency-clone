...
function menuInit() {
  ...
  if (!btn || !menu) return;
  const getNodes = () => {
    const infoContainer = menu.querySelector(INFO_CONTAINER_SELECTOR) || // Fallback: in case .menu__info is not in menu (rare, but)
    ...
    if (nodes.items.length) {
      tl.to(
        nodes.items,
        {
          ...
          duration: 0.3,
          stagger: 0.06
          // Bottom-to-top: stagger: { each: 0.06, from: 'end' }
        },
        0
      );
    }
    if (nodes.sections.length) {
      tl.to(
        nodes.sections,
        {
          ...
          duration: 0.35,
          stagger: 0.08
          // Bottom-to-top: stagger: { each: 0.08, from: 'end' }
        },
